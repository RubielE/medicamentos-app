...
export class QuimicaService {
  ...
  obtenerListaQuimica() {
   ...
   return this.httpClient.get(this.url+'/quimica/listar', {headers:header});
   //return this.httpClient.get("http://10.8.73.235:3300/quimica/listar", {headers:header});
  }

  //metodo crear reactivo
  crearReactivo(reactivo:quimica){
    let json = JSON.stringify(reactivo);
    let headers = new HttpHeaders({
        'Content-Type': 'application/json'
    });
    return this.httpClient.post(this.url+'/quimica/crear', json, {headers:headers})
   // return this.httpClient.post("http://10.8.73.235:3300/quimica/crear", json, { headers: headers })
      .pipe(
            map(res => {
      console.log(res);
       return res;
  ...
  editarReactivo (id:string, reactivo:quimica) {
    let json = JSON.stringify(reactivo);
    let params = json;
    let header = new HttpHeaders ({
      'Content-Type': 'application/json',
    ...
    return this.httpClient.put(this.url+'/quimica/editar/'+ id, params, {headers: header})
    //return this.httpClient.put("http://10.8.73.235:3300/quimica/editar" + id, json, {headers: Header})
    .map(res => res);
  }
