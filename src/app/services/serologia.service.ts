...
export class SerologiaService {
  ...
  obtenerListaSerologia() {
    let header = new HttpHeaders ( {
      "Content-Type": "application/json"
    });
   // mongodb://a-rubiel:enrique24@ds157735.mlab.com:57735/medicamentos'
   return this.httpClient.get(this.url+'/serologia/listar', {headers:header});
   //return this.httpClient.get("http://10.8.73.235:3300/serologia/listar", {headers:header});
  }

  //metodo crear reactivo
  crearReactivo(reactivo:serologia){
    let json = JSON.stringify(reactivo);
    let headers = new HttpHeaders({
        'Content-Type': 'application/json'
    });
    return this.httpClient.post(this.url+'/serologia/crear', json, {headers:headers})
   // return this.httpClient.post("http://10.8.73.235:3300/serologia/crear", json, { headers: headers })
      .pipe(
            map(res => {
      console.log(res);
       return res;
  ...
  editarReactivo (id:string, reactivo:serologia) {
    let json = JSON.stringify(reactivo);
    let params = json;
    let header = new HttpHeaders ({
      'Content-Type': 'application/json',
    ...
    return this.httpClient.put(this.url+'/serologia/editar/'+ id, params, {headers: header})
    //return this.httpClient.put("http://10.8.73.235:3300/serologia/editar" + id, json, {headers: Header})
    .map(res => res);
  }
